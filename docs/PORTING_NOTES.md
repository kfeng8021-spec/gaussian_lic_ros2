...
The CUDA rasterizer, fused SSIM loss, visibility-masked SparseGaussianAdam, densification/pruning, and strict CUDA rendered preview are ported behind the Torch/CUDA build profile. The native tracking package now provides timestamp-safe trajectory/IMU primitives, signed-nanosecond IMU history interpolation, raw-sample IMU preintegration/reintegration, IMU bias continuity residuals and marginalization-prior anchoring in the optional sliding window, bias observability status, per-point LiDAR deskew, native odometry/path/TF publication, bounded 6-DoF LiDAR residual correction, direct LiDAR/Gaussian-map point-to-point window factors, visual residual/direct image-alignment paths that compare mapper rendered images with incoming camera frames, analytic SE3 camera photometric pixel Jacobians with multi-sample normal-equation solving, optional SE3 photometric window factors, and chunk-complete `GaussianArray` snapshot caching for the mapper-to-tracker reverse channel. Full Coco-LIC2-grade production joint BA is still separate from this mapper backend work.
