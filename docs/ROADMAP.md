...
- Full native Coco-LIC2 frontend/tracking parity remains outside the current mapper-contract checkpoint.
...
- [x] Add an executable full-dataset strict parity evidence matrix.
  ...
  - Current local matrix status is `required=2/7`, `covered_profiles=fastlivo2`: the FAST-LIVO2 `CBD_Building_01` mapper-contract/CUDA strict report and CBD native visual/SE3 BA health report pass, while native reference trajectory parity plus FAST-LIVO/M2DGR/MCD/R3LIVE full-sequence strict artifacts are still missing.
...
- [x] Fix M2DGR native tracking time/QoS replay blockers before full strict parity.
  ...
  - `scripts/run_native_tracking_bag_report.sh` now exposes the LiDAR correspondence/correction/keyframe and sliding-window IMU/pose/smoothness weights used by the launch file, and `scripts/run_m2dgr_tracking_sweep.sh` ranks local M2DGR runs against `room_01_gt.tum`. A 2026-05-07 20s sweep at `results/m2dgr/room_01_tracking_sweep_20s/sweep_summary.json` currently ranks all-frame strict replay best by RMSE (`0.91 m`) while stronger IMU/pose/smoothness weighting lowers path drift but does not beat RMSE.
- [x] Add profile-aware ROS1 frontend-raw conversion and a data-side strict audit.
  ...
  - `scripts/audit_strict_data_inputs.py` writes `docs/strict_data_status.json` / `.md`, checks raw bags, converted frontend bags, ROS1 baselines, ROS2 current artifacts, trusted native reference trajectories, disk headroom, and the largest non-matrix `results/` cleanup candidates. The 2026-05-07 audit now separates raw/frontend/reference/baseline/current states: raw and frontend inputs pass for every required profile, M2DGR and MCD native reference TUM files are local, and the remaining blockers are ROS1 baseline artifacts plus FAST-LIVO/FAST-LIVO2/R3LIVE native reference trajectories.
- [x] Add generic Google Drive single-file fetcher and partial MCD data acquisition.
  ...
  - 2026-05-07 local MCD `ntu_day_01` acquisition now has `d435i` camera (`14,264,815,921` bytes), `mid70` LiDAR (`568,196,474` bytes), `vn100` IMU (`33,957,020` bytes), `groundtruth/pose_inW.tum`, frontend conversion, and ROS2 current runtime artifacts; the data audit deliberately keeps MCD failing until archived ROS1 baseline artifacts are present.
- [ ] Collect and pass full native reference trajectory parity for CBD and every required non-FAST-LIVO2 dataset/profile.
  - The attempted CBD comparison against `baseline/fastlivo2/CBD_Building_01/trajectory.tum` is not accepted as proof because that archived mapper-contract TUM is not a valid native frontend reference for the frontend-raw bag; the full-dataset matrix keeps this item failing until a trusted reference odometry/TUM source is available and passes `trajectory_compare.py`.
