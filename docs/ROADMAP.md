...
- [x] Add Coco-LIC-style LiDAR residual scale, limited-update auxiliary-state feedback, physical scan-to-scan target clamping, and optional fixed-control-point gauge anchoring to the continuous-time tracker.
  - 2026-05-13 CBD 60s ablations show path-length behavior changes, but long-window RMSE remains about 1.8 m, so the strict tracker parity item remains open.
- [x] Add retained-knot soft prior hooks for Coco-LIC-style fixed-prefix/marginalization ablations.
  - `TrajectoryEstimator` now supports direct knot position/orientation priors, `continuous_time_node` exposes `retained_knot_*` parameters, and the native parity script archives them.
  - 2026-05-13 CBD 12s probes confirm the hooks are live but not defaultable parity fixes: scan-to-scan position priors over-scale path length (`5.77 m` current vs `0.92 m` reference, RMSE `0.170 m`), retained-knot soft anchors over-scale further (`8.53 m`, RMSE `0.174 m`), and velocity-only scan priors reach RMSE `0.184 m`. The remaining blocker is a full Schur marginalization/global visual-map coupling, not simply a missing prefix anchor.
  - `lidar_scan_to_scan_relative_translation_gain` is archived by the native parity script so future gain sweeps can damp noisy ICP relative translation before it becomes a position/velocity prior. A 12s gain `0.15` probe still records RMSE `0.185 m` and path `5.97 m`, so this is a controlled diagnostic knob, not a solved parity setting.
- [x] Add native continuous-time 3D point-to-point residuals for persistent/Gaussian map anchors.
  - `TrajectoryEstimator` and `ContinuousTimeSlidingWindowEstimator` now accept coupled 3D LiDAR-to-map point residuals, and `continuous_time_node` routes persistent point-map matches through this path instead of three independent axis-plane pseudo-residuals.
  - Persistent point-map updates are now batched until scan end so current-scan points cannot become current-scan association targets.
  - CTest now covers both the batch estimator and online sliding-window consumption path. CBD 12s validation records the new residual active (`~12.5k` point factors) but still shortens path scale (`0.47 m` current vs `0.92 m` reference, RMSE `0.144 m`), so this is the correct residual interface for global Gaussian-map coupling, not completed tracker parity.
  - 2026-05-13 follow-up: persistent point-map entries now track first/last signed-nanosecond stamps, per-stamp observation counts, and same-stamp sample means. Matching skips immature anchors until `persistent_point_map_min_observations_for_match` and `persistent_point_map_min_match_age_s` are satisfied, and `continuous_time_native_reference_parity.sh` archives those knobs plus `persistent_point_map_merge_distance_m`. CBD 12s evidence shows this semantic hardening is not the missing parity constraint: `results/fastlivo2/CBD_Building_01_ct_point3d_mature_gate_12s/trajectory_compare_report.json` records RMSE `0.143944 m` and path `0.480/0.916 m`, while the same-stamp mean update at `results/fastlivo2/CBD_Building_01_ct_point3d_stamp_mean_12s/trajectory_compare_report.json` records RMSE `0.145313 m` and path `0.235/0.916 m`. The remaining 100% blocker is still long-window global trajectory-shape/visual-map coupling, not same-scan point-map pollution.
...
- [x] Add an executable full-dataset strict parity evidence matrix.
  ...
  - 2026-05-13 LiDAR scan-to-scan relative pose factors: `TrajectoryEstimator` now has two-timestamp relative position and relative SO(3) priors, and `ContinuousTimeSlidingWindow` buffers/promotes those factors without anchoring the global trajectory. The native node can route consecutive-scan ICP targets into these residuals via `lidar_scan_to_scan_use_relative_pose_factor`, and the parity script archives that flag. CTest covers both batch and streaming cases. CBD 12 s evidence shows the factor path is active but not a parity default: position+orientation weight `0.1` consumes 99/99 relative prior factors yet freezes path scale (`0.0355 m` vs `0.9159 m`, RMSE `0.144711 m`), and orientation-only still records path `0.0548 m` with RMSE `0.144454 m`. This closes the missing relative-pose residual surface; the remaining strict blocker is the quality/scale of the scan-to-scan target and the global visual/Gaussian-map coupling.
