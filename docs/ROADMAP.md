...
- [x] Add an executable full-dataset strict parity evidence matrix.
  ...
  - 2026-05-07: `scripts/run_strict_parity_queue.sh` now turns the remaining strict-data problem into an executable queue across FAST-LIVO, FAST-LIVO2, M2DGR, MCD, and R3LIVE. It performs profile-aware frontend conversion, ROS1 mapper-contract export, profile-matched upstream baseline runs, ROS2 CUDA current collection, strict reports, and data/matrix audits. This creates the missing evidence path; the matrix stays incomplete until those long runs finish and pass.
