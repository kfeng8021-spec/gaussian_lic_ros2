...
- [x] Add an executable full-dataset strict parity evidence matrix.
  ...
  - 2026-05-12 continuous-time Ceres tuning: FAST-LIVO2 frontend_raw IMU samples were verified as normalized-g (`|a| ~= 0.991`) and the standalone continuous-time tracker now exposes `imu_linear_acceleration_scale`, `imu_info_gyro`, and `imu_info_accel`; the parity script defaults to `9.80665`, one online Ceres iteration, and `10/1` IMU information weights. This moves the CBD prior-seeded smoke from the previous 12 s RMSE `10.541 m` to `0.559 m` at `results/fastlivo2/CBD_Building_01_continuous_time_scaled_iter1_prior_smoke_12s/trajectory_compare_report.json`; a 4 s tuning run reaches `0.164 m` RMSE at `results/fastlivo2/CBD_Building_01_continuous_time_scale_iter1_g10_a1_prior_smoke_4s/trajectory_compare_report.json`. Full-sequence RMSE-gated parity is still the open target.
