...
- Latest short CBD continuous-time smoke, 2026-05-12: the pre-guard persistent-plane path produced a 4 s prior-seeded RMSE of about `144103 m`; after solve-step rejection the same 4 s prior-seeded smoke is finite with `41` TUM poses, `34` reference matches, RMSE `2.725 m`, and max error `4.591 m` at `results/fastlivo2/CBD_Building_01_continuous_time_solver_guard_prior_smoke_4s/trajectory_compare_report.json`. With LiDAR Huber loss, mature persistent-plane gating, and the data-backed default `pointcloud_factor_weight=0.1`, the 4 s prior-seeded plane-on smoke improves to RMSE `1.156 m`, mean `1.000 m`, max `1.945 m` at `results/fastlivo2/CBD_Building_01_continuous_time_plane_mature_huber_weight01_prior_smoke_4s/trajectory_compare_report.json`. The current continuous-time tuning fixes FAST-LIVO2 normalized-g IMU scaling (`9.80665`) and limits online Ceres to one iteration with lower IMU information weights; the 4 s prior-seeded smoke reaches RMSE `0.164 m`, mean `0.141 m`, max `0.282 m` at `results/fastlivo2/CBD_Building_01_continuous_time_scale_iter1_g10_a1_prior_smoke_4s/trajectory_compare_report.json`, and the 12 s prior-seeded smoke reaches RMSE `0.559 m`, mean `0.482 m`, max `1.139 m` at `results/fastlivo2/CBD_Building_01_continuous_time_scaled_iter1_prior_smoke_12s/trajectory_compare_report.json`. These are hardening checkpoints, not full-sequence paper parity.
