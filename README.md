...
- Tracking step-guard diagnostics now report clamp counts plus the last raw/allowed step, dt, and reference speed. Adaptive step-guard knobs are exposed through `tracking.launch.py` and `run_native_tracking_bag_report.sh`, but remain opt-in: a 30 s CBD probe with velocity/acceleration scaling proved the parameters are live by raising current step p95 to `0.073 m`, but it overdrives path length to `10.30 m` vs `2.82 m` reference and worsens RMSE to `0.674 m`. The Gaussian feedback preset therefore uses the measured fixed `0.025 m` guard for rasterizer feedback until a confidence-gated speed model is proven on full windows.
