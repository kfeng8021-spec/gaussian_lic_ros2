...
- Native tracking reference comparisons now archive 8 time-ordered error bins by default through `run_native_tracking_bag_report.sh --reference-error-bin-count` and run a default `[-0.5, 0.5] s` timestamp-offset sweep through `--reference-time-offset-sweep`, so each report shows where late-window drift starts and whether it is just a replay-time offset. The latest 60 s CBD factor-weight probes make native LiDAR plane weight the best new preset lever: plane weight `2.0` improves to `0.792 m` RMSE, `4.0` records `0.750 m` then `0.723 m` on repeat, and the no-explicit-flag default confirmation records `0.825 m`; `8.0` records the best single run (`0.705 m`) but does not reproduce as a stable default (`1.006 m` on confirmation); `16.0` regresses to `0.799 m`, point weight `2.0` gives `0.822 m`, and point+plane `2.0` regresses to `1.201 m`. The Gaussian feedback preset therefore conservatively promotes `lidar_window_plane_factor_weight=4.0`. A new default-off adjacent-state relative-translation BA factor is now wired from raw IMU/pre-LIO motion into the same sliding window and archived by the native report; CBD 60 s probes show it is useful but not stable enough for default (`weight=0.25 -> 0.773 m`, `weight=0.1 -> 0.702 m` then `0.833 m` on repeat). Gaussian snapshot plane anchors, low-frequency CUDA optimization, and position-velocity consistency remain non-default after 60 s probes (`0.863 m`, `0.789 m`, and `1.068 m`). The full120 CBD replay at `results/fastlivo2/CBD_Building_01_gaussian_feedback_plane4_full120_probe/native_tracking_report.json` improves the older capped full120 RMSE from `2.488 m` to `2.345 m` with `95.29%` coverage and `22.09%` path drift, but the error bins still expose a long-window trajectory-shape failure (`3.44/3.46 m` RMSE in bins 5/6, about `-3 m` Y bias). This is not strict paper parity yet.
