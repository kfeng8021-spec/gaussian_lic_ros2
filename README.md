...
- Native tracking package with signed-nanosecond trajectory and IMU primitives, IMU history interpolation, raw-sample IMU preintegration/reintegration, IMU bias continuity and marginalization-prior anchoring inside the optional sliding window, per-point LiDAR deskew, bounded 6-DoF LiDAR residual correction, direct LiDAR/Gaussian-map point-to-point window factors, visual residual comparison against mapper renders, chunk-complete GaussianMap snapshot caching, odometry/path/optional TF publication, and deterministic probes.
...
- Full Coco-LIC2-grade frontend/tracking algorithm: production sliding-window joint BA, VIO Jacobians, robust IMU bias observability from LIO/VIO factors, and Schur-complement marginalization.
...
- The native tracker has timestamp-safe trajectory/IMU primitives, IMU history interpolation, raw-sample IMU preintegration/reintegration, IMU bias continuity residuals, marginalization-prior anchoring, per-point LiDAR deskew, direct LiDAR/Gaussian-map point-to-point window factors, chunk-complete Gaussian snapshot caching, an optional sliding-window optimizer foundation, and an initial visual residual path, but it is not yet the full continuous-time Coco-LIC2 production BA stack.
- VIO residual Jacobians, robust IMU bias observability from LIO/VIO factors, true Schur-complement marginalization, and full joint BA remain to be ported beyond the current deskewed bounded 6-DoF LiDAR correction and finite-difference IMU-window foundation.
...
These profiles currently cover the native mapper surface: topic names, QoS, synchronization, upstream camera intrinsics, image size, depth-completion toggles, Gaussian initialization/map-growth parameters, upstream optimizer/loss/exposure parameter names, output topics, and active profile labels. The native tracking launch also exposes per-point LiDAR deskew controls for common `offset_time`, `time`, `timestamp`, and `t` fields while preserving signed-nanosecond estimator math. The optional sliding-window optimizer is available behind `enable_sliding_window_optimizer:=true` for IMU-factor development. Full real-dataset frontend parity still needs Coco-LIC2-grade production BA, VIO Jacobians, IMU bias optimization, and marginalization.
