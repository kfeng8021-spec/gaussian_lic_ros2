...
- Native tracking package with signed-nanosecond trajectory and IMU primitives, signed-nanosecond image/LiDAR/IMU status gates, bounded best-effort tracking sensor QoS exposed through launch/status gates, executor callback serialization for ROS1-style estimator mutation order, IMU history interpolation, raw-sample IMU preintegration/reintegration, optimized velocity/bias feedback into odometry and safe IMU re-anchoring, IMU bias continuity and marginalization-prior anchoring inside the optional sliding window, bias observability status/probes, reusable normal-equation linearization, Schur-complement retained-window dense priors with rank/singular-value health, timestamp-safe cubic B-spline pose/velocity trajectory queries for deskew fallback, per-point LiDAR deskew, bounded 6-DoF LiDAR residual correction, robust direct LiDAR/Gaussian-map point-to-point window factors, LiDAR point-to-plane window factors, analytic geometric Jacobians for point-to-point, point-to-plane, visual-alignment, and full current IMU preintegration factors, visual residual/direct subpixel image-alignment comparison against mapper renders, runtime-gated 2-DoF photometric translation linearization, analytic SE3 camera photometric pixel Jacobians, multi-sample normal equations, robust runtime rendered/current/depth sampling into optional SE3 photometric window factors, optional visual-alignment window factors, chunk-complete GaussianMap snapshot caching, odometry/path/status/optional TF publication, and deterministic probes.
...
- Native tracking probes are registered with CTest, so `colcon test --packages-select gaussian_lic_tracking` runs trajectory, IMU, LiDAR, sliding-window, bias observability, geometric Jacobian, Gaussian snapshot, SE3 photometric Jacobian/factor, and visual checks automatically. `scripts/tracking_smoke_test.sh` also verifies the launch path through `/gaussian_lic/frontend/status`, including signed-nanosecond time status, tracking QoS, executor callback serialization, dense-prior health, bias observability, visual alignment, photometric linearization status, and SE3 photometric sample quality.
...
- The native tracker has timestamp-safe trajectory/IMU primitives, IMU history interpolation, raw-sample IMU preintegration/reintegration, optimized velocity/bias feedback into odometry and safe IMU re-anchoring, IMU bias continuity residuals, dense marginalization-prior anchoring, bias observability metrics/probes, reusable normal-equation linearization, Schur-complement retained-window dense priors, timestamp-safe cubic B-spline pose/velocity trajectory queries for deskew fallback, per-point LiDAR deskew, robust direct LiDAR/Gaussian-map point-to-point window factors, LiDAR point-to-plane window factors, analytic geometric Jacobians for point-to-point, point-to-plane, visual-alignment, full current IMU preintegration factors including start-bias reintegration sensitivity, pose-prior, state-prior, dense-prior, IMU bias-continuity, and SE3 photometric window rows, analytic SE3 camera photometric pixel Jacobians with multi-sample normal-equation solving, runtime rendered/current/depth extraction into default-enabled SE3 photometric window factors, chunk-complete Gaussian snapshot caching, a default-enabled sliding-window optimizer foundation, direct visual residual/subpixel alignment, runtime-gated photometric translation linearization, and default-enabled visual-alignment window factors, but it is not yet the full continuous-time Coco-LIC2 production BA stack.
...
These profiles currently cover the native mapper surface: topic names, QoS, synchronization, upstream camera intrinsics, image size, depth-completion toggles, Gaussian initialization/map-growth parameters, upstream optimizer/loss/exposure parameter names, output topics, and active profile labels. The native tracking launch also exposes sensor QoS controls, keeps high-rate tracking inputs at bounded `best_effort` by default, exposes per-point LiDAR deskew controls for common `offset_time`, `time`, `timestamp`, and `t` fields while preserving signed-nanosecond estimator math, publishes latest signed-nanosecond image/LiDAR/IMU stamps for smoke gating, and serializes tracking callbacks by default so a future multi-threaded executor cannot concurrently mutate IMU/LiDAR/image estimator state. The sliding-window optimizer is now default-enabled for production BA runtime coverage, with reusable normal-equation linearization, dense prior anchoring, Schur-complement retained-window priors with rank/singular-value health, optimized velocity/bias feedback into odometry and safe IMU re-anchoring, timestamp-safe cubic B-spline pose/velocity trajectory queries for deskew fallback, bias observability status, per-correspondence robust LiDAR/Gaussian-map weights, LiDAR point-to-plane factors, analytic geometric and full current IMU preintegration Jacobians, robust analytic SE3 camera photometric normal equations/window factors from rendered/current/depth images, runtime status for 2-DoF and SE3 photometric linearization quality, and subpixel visual alignment in place. Full real-dataset frontend parity still needs Coco-LIC2-grade production BA.
