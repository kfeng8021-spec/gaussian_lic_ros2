...
- Full Coco-LIC2-grade frontend/tracking algorithm: production sliding-window joint BA, production-scale photometric sampling policy, and production-scale marginalization policy.
...
- The native tracker has timestamp-safe trajectory/IMU primitives, IMU history interpolation, raw-sample IMU preintegration/reintegration, IMU bias continuity residuals, dense marginalization-prior anchoring, bias observability metrics/probes, reusable normal-equation linearization, Schur-complement retained-window dense priors, per-point LiDAR deskew, robust direct LiDAR/Gaussian-map point-to-point window factors, LiDAR point-to-plane window factors, analytic geometric Jacobians for point-to-point, point-to-plane, and visual-alignment factors, analytic SE3 camera photometric pixel Jacobians with multi-sample normal-equation solving, runtime rendered/current/depth extraction into optional SE3 photometric window factors, chunk-complete Gaussian snapshot caching, an optional sliding-window optimizer foundation, direct visual residual/subpixel alignment, runtime-gated photometric translation linearization, and optional visual-alignment window factors, but it is not yet the full continuous-time Coco-LIC2 production BA stack.
- Full joint BA and production-scale marginalization remain to be ported beyond the current deskewed bounded 6-DoF LiDAR correction, 2-DoF visual translation linearization gate, SE3 photometric window-factor runtime gate, bias observability gate, and finite-difference IMU-window foundation.
...
These profiles currently cover the native mapper surface: topic names, QoS, synchronization, upstream camera intrinsics, image size, depth-completion toggles, Gaussian initialization/map-growth parameters, upstream optimizer/loss/exposure parameter names, output topics, and active profile labels. The native tracking launch also exposes sensor QoS controls, keeps high-rate tracking inputs at bounded `best_effort` by default, exposes per-point LiDAR deskew controls for common `offset_time`, `time`, `timestamp`, and `t` fields while preserving signed-nanosecond estimator math, publishes latest signed-nanosecond image/LiDAR/IMU stamps for smoke gating, and serializes tracking callbacks by default so a future multi-threaded executor cannot concurrently mutate IMU/LiDAR/image estimator state. The optional sliding-window optimizer is available behind `enable_sliding_window_optimizer:=true` for IMU-factor development, with reusable normal-equation linearization, dense prior anchoring, Schur-complement retained-window priors with rank/singular-value health, bias observability status, per-correspondence robust LiDAR/Gaussian-map weights, LiDAR point-to-plane factors, analytic geometric Jacobians, robust analytic SE3 camera photometric normal equations/window factors from rendered/current/depth images, runtime status for 2-DoF and SE3 photometric linearization quality, and subpixel visual alignment in place. Full real-dataset frontend parity still needs Coco-LIC2-grade production BA and production-scale marginalization policy.
