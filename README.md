...
- Native tracking evidence now records online `first`/`last`/`min`/`max`/`mean`/`delta` summaries for every numeric `TrackingStatus` and mapper `MappingStatus` field in `metrics.json`. This turns each bag report into a time-series health artifact for step-guard clamps, visual/SE3 quality, Gaussian snapshot growth, mapper optimization counters, and BA conditioning instead of relying on only the final status sample.
...
- Direction-agreement step-guard release is the current best native CBD trajectory-shape checkpoint and is promoted into the `--enable-gaussian-map-feedback` preset. With max6/light relative rotation plus `post_ba_step_guard_pre_ba_agreement_max_pose_step_m=0.040`, `min_cosine=0.95`, and `max_delta=0.02`, full122 reaches `1087` matches, `88.30%` coverage, `1.878 m` RMSE, mean `1.783 m`, max `3.211 m`, and `3.15%` path drift at `results/fastlivo2/CBD_Building_01_gaussian_feedback_step00265_relrot001_mhrot0005_mhtrans003_max6_agree0040_full122_probe/native_tracking_report.json`, with numeric Jacobian max `0`. The stricter `0.035` release lowers path drift to `6.08%` but has RMSE `2.259 m`; the looser `0.045` release lowers drift to `1.34%` but raises RMSE to `1.962 m`; the full125 replay of `0.040` regresses to RMSE `2.457 m` despite `2.50%` drift. This is a substantial improvement over the previous `1.895 m / 20.73%` best, but not yet a 100% paper/super-paper parity claim.
...
`2.858 m` max error, and `2.26%` path drift. It uses the per-feedback gyro/accel
bias step clamps (`0.02` / `0.12`) now promoted by the Gaussian-feedback report
preset while remaining off in the raw node/launch defaults; simple time-scaled
bias random-walk weighting, stronger clamps, adjacent `0.015`/`0.025` gyro
step sweeps, and a wider `0.14` accel step were rejected by full-window evidence. The diagnostic plots still
