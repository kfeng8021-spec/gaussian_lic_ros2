...
- Gaussian-feedback strict replay now promotes the mapper image input QoS to `reliable` with depth `512` unless the caller explicitly overrides `--mapper-feedback-image-qos-reliability` or `--mapper-feedback-image-qos-depth`. The live/default mapper profile remains `best_effort`; the strict replay override is evidence-driven by full122 runs where the mapper received only `900/1187` camera frames under best-effort and could not reach the visual/SE3 drain target.
...
mapper image QoS, so strict replay now promotes mapper image QoS to
`reliable`/depth `512` while leaving live defaults explicitly overrideable.
