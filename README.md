...
- Native tracking drift diagnostics can now additionally write per-sample compact status history with `run_native_tracking_bag_report.sh --write-status-history` and post-process it through `scripts/diagnose_native_tracking_drift.py`. The diagnostic emits bias trajectory, factor-count, Hessian-health, and reference-motion-regime CSV/PNG artifacts. The 2026-05-14 CBD full120 diagnosis found that widening the active window to 30 states is not a production fix because it only processed `103` poses before replay shutdown; the production fix instead delays window marginalization until the current frame's state priors, IMU, LiDAR, visual, SE3, and relative factors are all inserted. The regression probe now verifies that the retained dense prior remains full-rank (`45/45`) when a new current-frame prior would previously have been missed.
